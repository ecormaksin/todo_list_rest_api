...
public interface TaskRepository extends JpaRepository<Task, Integer> {
	...
	@Query("SELECT x FROM Task x ORDER BY x.title, x.detail, x.id")
	Page<Task> findAllOrderByTitle(Pageable pageable);
	
	// https://stackoverflow.com/questions/21456494/spring-jpa-query-with-like
	@Query("SELECT x FROM Task x WHERE x.title LIKE CONCAT('%',:keyword,'%') OR x.detail LIKE CONCAT('%',:keyword,'%') ORDER BY x.title, x.detail, x.id")
